...
class TCS3472 {
  public:
    ...
    void setTime(tcs_time_t time) { _write8(TCS_ATIME, (uint8_t)time); }

    void setGain(tcs_gain_t gain) { _write8(TCS_CONTROL, (uint8_t)gain); }

    bool status() { return _ok; }
  ...
  private:
    ...
    void _endCMD() { _wire->endTransmission(0); }
};
