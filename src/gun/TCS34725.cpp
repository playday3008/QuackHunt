...
void TCS34725::setIntegrationtime(eIntegrationTime_t it)
{
  uint8_t data = it;
...
void TCS34725::setGain(eGain_t gain)
{
  uint8_t data = gain;
...
void TCS34725::setGenerateinterrupts(void)
{
  uint8_t data = TCS34725_PERS_NONE;
  writeReg(TCS34725_PERS, &data,1);
}

void TCS34725::writeReg(uint8_t Reg, void *pData, uint8_t len)
{
...
int16_t TCS34725::readReg(uint8_t Reg, uint8_t *Data, uint8_t len)
{
  int i = 0;
  _pWire->beginTransmission(this->_I2C_addr);
  _pWire->write(TCS34725_COMMAND_BIT | Reg);
